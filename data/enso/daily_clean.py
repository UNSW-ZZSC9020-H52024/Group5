...
#It then creates a new column 'enso' that is 1 for el nino, -1 for la nina, and 0 for enso neutral based on the BoM criteria (http://www.bom.gov.au/climate/updates/articles/a008-el-nino-and-australia.shtml)
...
sst[(sst['DATETIME'] <= '2021-03-18')]
soi_load = soi_load[(soi_load['DATETIME'] >= '2009-12-31')]
soi_load[(soi_load['DATETIME'] <= '2021-03-18')]
...
#using the 3m average for the bom criteria gives 0 for all values, so we will use the daily SOI values instead
...
#function to compute enso based on the BoM criteria
...
# function to compute enso based on the SOI only
...
print("value counts of the 'enso' column (soi only)")
print(soi_load.value_counts('enso'))
print("value counts of the 'enso' column (BOM criteria)")
