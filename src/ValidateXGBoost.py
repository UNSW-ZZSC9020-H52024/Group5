...
def main():
    ...
    model_save_path = '../data/NSW/saved_tuned_xgb_model.pkl'
    with open(model_save_path, 'wb') as f:
        pickle.dump(best_xgb, f)
    print(f"Tuned XGBoost model saved to {model_save_path}")
    
    # Predict for daily, weekly, monthly, and yearly demand
    y_pred_daily = best_xgb.predict(X_daily)
    y_pred_weekly = best_xgb.predict(X_weekly)
    y_pred_monthly = best_xgb.predict(X_monthly)
    y_pred_yearly = best_xgb.predict(X_yearly)
    ...
    plt.xlabel('Date')
    plt.ylabel('Demand')
    plt.title('Daily Energy Demand vs. Predicted Demand (XGBoost)')
    plt.legend()
    plt.show()
